...
same `workspace/results/runs/*/results_val.csv` files `pca_model_redundancy.py`
reads (via `select_finalist_from_val`'s loader), used only to rank which
member of a redundant cluster to keep (the official hd95_mm/dice ranking,
same rule used everywhere else in this project -- not re-derived from
probabilities).
...
`workspace/results/finalist_selection/probability/` -- a subdirectory, so it
never collides with `pca_model_redundancy.py`'s case-Dice outputs of the
same filenames):
...
Usage:
    python analysis/finalist_selection/pca_probability_redundancy.py
    python analysis/finalist_selection/pca_probability_redundancy.py --redundancy-corr-threshold 0.9
"""
...
def main() -> None:
    ...
    ap.add_argument("--in-csv", default="workspace/results/finalist_selection/voxel_probability_correlation.csv", type=Path)
    ap.add_argument("--runs-dir", default="workspace/results/runs", type=Path)
    ap.add_argument("--out-dir", default="workspace/results/finalist_selection/probability", type=Path)
