...
from .sampling_utils import curriculum_power, inverse_volume_probabilities, sampling_diagnostics
...
class _LesionVolumeSamplingBase(nnUNetTrainer):
    ...
    def get_dataloaders(self):
        ...
        def weighted_loader(*args, **kwargs):
            nonlocal construction_count, sampled_case_count, active_probabilities
            ...
            if construction_count == 1:
                ...
                active_probabilities = self._probabilities_for_identifiers(identifiers)
                kwargs["sampling_probabilities"] = active_probabilities
        ...
        if construction_count < 2 or sampled_case_count == 0 or active_probabilities is None:
        ...
        diagnostics = sampling_diagnostics(active_probabilities)
        ...
        self.print_to_log_file(
            ...
            f"phase={phase_name}, inverse-volume power={power:.2f}, "
            f"p_min={diagnostics['min_probability']:.6g}, p_max={diagnostics['max_probability']:.6g}, "
            f"max/min={diagnostics['max_to_min_ratio']:.2f}, "
            f"effective_n={diagnostics['effective_sample_size']:.1f}. Validation remains uniform."
        )
...
class nnUNetTrainerCurriculumLesionAwareSampling(_LesionVolumeSamplingBase):
    """Progressively increase small-lesion sampling emphasis (experiment 5)."""
