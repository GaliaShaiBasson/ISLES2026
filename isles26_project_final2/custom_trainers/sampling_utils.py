...
def sampling_diagnostics(probabilities) -> dict[str, float]:
    """Summarize how concentrated a case-sampling distribution is."""
    ...
    return {
        "min_probability": float(normalized.min()),
        "max_probability": float(normalized.max()),
        "max_to_min_ratio": float(normalized.max() / positive.min()),
        "effective_sample_size": float(1.0 / np.square(normalized).sum()),
    }
