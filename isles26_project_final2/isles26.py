...
def _source_hashes() -> dict[str, str]:
    ...
    for path in sorted(PROJECT_ROOT.rglob("*.py")):
        if "workspace" in path.parts or "__pycache__" in path.parts:
            continue
...
def write_run_manifest(
    ...
    """Record enough context to audit and reproduce a training run."""
    timestamp = datetime.now(timezone.utc)
    output_dir = Path(env["ISLES26_WORKSPACE"]) / "run_manifests"
    output_dir.mkdir(parents=True, exist_ok=True)
    safe_fold = re.sub(r"[^A-Za-z0-9_.-]", "_", str(fold))
    output_path = output_dir / f"{experiment}_fold{safe_fold}_{timestamp.strftime('%Y%m%dT%H%M%SZ')}.json"
    payload = {
        ...
        "packages": {
            ...
            for name in ("nnunetv2", "torch", "numpy", "scipy", "pandas", "nibabel", "matplotlib")
        },
        "configuration_values": {key: env.get(key) for key in DEFAULTS},
        "source_sha256": _source_hashes(),
    }
    output_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"Run manifest: {output_path}")
    return output_path
...
def _torch_version_supported(version: str) -> bool:
    ...
    numeric = [int(value) for value in re.findall(r"\d+", base)[:3]]
    if len(numeric) < 2:
        raise ValueError(f"Unreadable PyTorch version: {version}")
    while len(numeric) < 3:
        numeric.append(0)
    parsed = tuple(numeric)
    return parsed >= (2, 1, 2) and parsed[:2] != (2, 9)
...
def cmd_make_splits(args: argparse.Namespace) -> int:
    ...
    dataset_id, dataset_name = _dataset_id_name(env, args.dataset_id, args.dataset_name)
    metadata = metadata_csv_path(env, dataset_id, dataset_name)
    output = Path(env["nnUNet_preprocessed"]) / dataset_folder_name(dataset_id, dataset_name) / "splits_final.json"
    summary = Path(env["ISLES26_METADATA_DIR"]) / f"{dataset_folder_name(dataset_id, dataset_name)}_splits_summary.csv"
    command = [
        sys.executable,
        str(PROJECT_ROOT / "data_prep" / "create_stratified_splits.py"),
...
def cmd_train(args: argparse.Namespace) -> int:
    ...
    ensure_workspace(env)
    dataset_id, dataset_name = _dataset_id_name(env, args.dataset_id, args.dataset_name)
    folds = [str(value) for value in args.folds] if args.folds else [
        str(args.fold if args.fold is not None else config_value(env, "ISLES26_FOLD"))
    ]
    ...
    for experiment in experiments:
        ...
        for fold in folds:
            print(f"\n== {experiment}: {trainer}, fold {fold} ==")
            _train_one(experiment, trainer, dataset_id, dataset_name, configuration, fold, device, num_gpus, args, env)
    return 0
...
def cmd_evaluate(args: argparse.Namespace) -> int:
    ...
    folds = [str(value) for value in args.folds] if args.folds else [str(config_value(env, "ISLES26_FOLD"))]
    if not args.from_training_validation and len(folds) > 1:
        raise SystemExit("Multiple folds require --from-training-validation")
    if args.pred_dir and args.from_training_validation:
        raise SystemExit("--pred-dir and --from-training-validation are mutually exclusive")
    if args.out_csv and len(folds) > 1:
        raise SystemExit("--out-csv cannot be used with multiple folds")

    configuration = args.configuration or config_value(env, "ISLES26_CONFIGURATION")
    trainer = args.trainer or EXPERIMENT_TRAINERS.get(args.experiment, args.experiment)
    splits_json = Path(env["nnUNet_preprocessed"]) / dataset_folder_name(dataset_id, dataset_name) / "splits_final.json"
    for fold in folds:
        if args.from_training_validation:
            result_root = (
                Path(env["nnUNet_results"])
                / dataset_folder_name(dataset_id, dataset_name)
                / f"{trainer}__{args.plans_name}__{configuration}"
                / f"fold_{fold}"
                / "validation"
            )
            pred_dir = result_root
            out_csv = out_dir / f"results_{slug}_fold{fold}.csv"
        else:
        ...
        if args.from_training_validation:
            command += ["--splits-json", str(splits_json), "--fold", fold, "--fold-label", fold]
        elif args.expected_cases_file:
            command += ["--expected-cases-file", str(Path(args.expected_cases_file).expanduser().resolve())]
...
def _aggregate_command(env: dict[str, str], inputs: list[Path]) -> list[str]:
    ...
    return [
        ...
        *[str(p) for p in inputs],
        ...
        "--out-paired", str(results_dir / "paired_comparisons.csv"),
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("make-splits", help="Create deterministic subject-grouped, center/size-aware CV folds")
    p.add_argument("--dataset-id", type=int)
    p.add_argument("--dataset-name")
    ...
    p.add_argument("--dataset-id", type=int)
    p.add_argument("--dataset-name")
    folds = p.add_mutually_exclusive_group()
    folds.add_argument("--fold")
    folds.add_argument("--folds", nargs="+", help="Run several CV folds sequentially, for example 0 1 2 3 4")
    ...
    p.add_argument("--from-training-validation", action="store_true", help="Evaluate strict out-of-fold validation outputs")
    p.add_argument("--folds", nargs="+", help="Validation folds; requires --from-training-validation for multiple folds")
    p.add_argument("--configuration")
    p.add_argument("--plans-name", default="nnUNetPlans")
    p.add_argument("--trainer", help="Trainer class override used to locate validation outputs")
    p.add_argument("--expected-cases-file", help="Explicit case list for a non-CV prediction folder")
    p.add_argument("--allow-missing", action="store_true", help="Debug only: permit incomplete expected cohorts")
    p.add_argument("--dataset-id", type=int)
    p.add_argument("--dataset-name")
