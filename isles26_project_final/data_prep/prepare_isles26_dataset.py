...
def main() -> int:
    ...
    if args.dry_run:
        preview_count = min(10, len(cases))
        print(f"Dry run only: showing {preview_count} of {len(cases)} discovered cases. A real prepare run uses all cases.")
        for case_id, info in list(cases.items())[:preview_count]:
